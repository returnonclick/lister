...
function display_cff($atts, $content = null) {
    ...
    if ( ($cff_events_only && !$cff_is_group) || $cff_photos_only || ($cff_featured_post_active && !empty($atts['featuredpost'])) ) $access_token = $access_token_array_fql[rand(0, 4)];
    ...
    foreach ( $page_ids as $page_id ) {
        ...
        if ($cff_events_only && $cff_events_source == 'eventspage'){
            ...
            //Events URL
            $cff_events_json_url = "https://graph.facebook.com/v2.5/".$u_id."/events/?fields=id,name,attending_count,cover,start_time,end_time,timezone,place,description,ticket_uri".$cff_start_time_string."&limit=200&access_token=" . $access_token . "&format=json-strings" . $cff_ssl;
            ...
            if($cff_past_events) $cff_events_json_url = 'https://graph.facebook.com/v2.5/'.$u_id.'/events?fields=name,id,description,start_time,end_time,timezone,place,ticket_uri,cover&limit=200&until='.date('Y-m-d').'&access_token='.$access_token;

            //Group events
            if($cff_is_group && !$cff_past_events) $cff_events_json_url = 'https://graph.facebook.com/v2.5/' . $page_id . '/events?fields=name,id,description,start_time,end_time,timezone,ticket_uri,place,cover&limit=200&since='.date('Y-m-d').'&access_token=' . $access_token;

            //Featured Post extension
            if( $cff_featured_post_active && !empty($atts['featuredpost']) ) $cff_events_json_url = "https://graph.facebook.com/fql?q=" . cff_featured_event_id( trim( $atts['featuredpost'] ), $access_token );
            ...
            if( empty($event_data->data) ){
                ...
                foreach ($event_data->data as $event )
                {
                    ...
                    //Featured post still uses FQL so uses eid
                    if( $cff_featured_post_active && !empty($atts['featuredpost']) ) isset($event->eid) ? $id = $event->eid : $id = '';
        ...
        if (!$cff_events_only || ($cff_events_only && $cff_events_source == 'timeline') ){
            ...
            //VIDEOS ONLY
            if($cff_videos_only){
                $cff_posts_json_url = 'https://graph.facebook.com/v2.5/'.$page_id.'?fields=videos{source,title,description,embed_html,format{picture}}&access_token='.$access_token.'&locale='.$cff_locale . $cff_date_range;
            }
            ...
            if( ( empty($FBdata->data) && empty($FBdata->videos) ) && (!$cff_featured_post_active || empty($atts['featuredpost'])) && !$cff_ext_multifeed_active ) {
                $cff_content .= '<div class="cff-error-msg"><p>Unable to display Facebook posts <br /><a href="javascript:void(0);" id="cff-show-error" onclick="cffShowError()">Show Error Message</a></p>';
                $cff_content .= '<script type="text/javascript">function cffShowError() { document.getElementById("cff-error-reason").style.display = "block"; document.getElementById("cff-show-error").style.display = "none"; }</script>';
                $cff_content .= '</p><div id="cff-error-reason">';
                        ...
                        if($cff_offset_show_post){
                            ...
                            if($cff_photos_only && empty($cff_album_id)){
                                ...
                                if( $cff_show_post ){
                                    ...
                                    //Fix Photon (Jetpack) issue
                                    $cff_picture_querystring = '';
                                    if (parse_url($news->src_big, PHP_URL_QUERY)){
                                        $picture_url_parts = parse_url($news->src_big);
                                        $cff_picture_querystring = $picture_url_parts['query'];
                                    }

                                    if( $columns_compatible ) {
                                        $cff_post_item .= '<div class="cff-album-item cff-col-'.$cff_photos_cols.'" id="cff_'. $news->pid .'">';
                                        $cff_post_item .= '<a href="'.$news->link.'" class="cff-album-cover" '.$target.$cff_nofollow.'><img src="'. $news->src_big .'" alt="'.$cff_caption.'" data-querystring="'.$cff_picture_querystring.'" /></a>';
                                        $cff_post_item .= '</div>';
                                    } else {
                                        $cff_post_item .= '<div class="cff-album-item" id="cff_'. $news->pid .'">';
                                        $cff_post_item .= '<a href="'.$news->link.'" class="cff-album-cover" '.$target.$cff_nofollow.'><img src="'. $news->src_big .'" alt="'.$cff_caption.'" data-querystring="'.$cff_picture_querystring.'" /></a>';
                                        $cff_post_item .= '</div>';
                                    }
                                }
...
function cff_custom_css() {
    ...
    echo "\r\n";
    //Add the absolute path to be used in the connect.php file for group albums:
    echo 'var cffabspath = "' . str_replace("\\", "\\\\", ABSPATH) . '";';
    echo "\r\n";
    echo '</script>';
    echo "\r\n";
}
