...
* *Common Hashtags Feed Support:* By default Instagram shows all the feeds from the multiple hashtags. Right now instagram did not have a feature to show feeds common in the multiple hastags. The pro version does have a feature to display only those feeds common in the multiple hashtags. 
...
Always check for *javascript issue* in the console. Plugin heart is javascript so if you have any js conflict prior to the instagram plugin activation irrespective of any theme and plugin than it will not work. If no js issue is found than always check the *access token* validity. To test access token place your token in place of dummy token mentione in the url *https://api.instagram.com/v1/users/self/feed?access_token=ACCESS-TOKEN*. If you get this message *The access_token provided is invalid* than you have re-authenitcate this plugin with the instagram account and get new access token.
