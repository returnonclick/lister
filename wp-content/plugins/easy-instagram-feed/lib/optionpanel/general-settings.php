...
<table class="form-table">
        <tr valign="top"><span style="color:red;font-size:16px;">Due to Instagram API changed on 1st june 2016 the authorize button will not work. We are working with the New API and soon release the updated version. To fetch feeds new users can use the default token. Kindly don't change this token atleast till the next plugin update. </span>
        <th scope="row"><label>Your Access Token </label></th> 
        <td><input type="text" id="eif_access_token"  name="eif_access_token" value="<?php esc_attr_e($eif_settings['eif_access_token']); ?>" /><span>Default Access Token</span></td>
        </tr>
         
        <tr valign="top">
        <th scope="row"><label>User Id</label></th>
        <td><input type="radio" name="search" value="user" checked><input type="text" id="eif_user_id" name="eif_user_id" value="<?php esc_attr_e(implode(',',$eif_settings['eif_user_id'])); ?>" /><span style="font-style:Italic;"> Seperate multiple Ids using commas </span><a href='http://webriti.com/instagramuserid/instagramuserid.php' target='_blank'>get user id </a> </td>
        </tr>
		<tr valign="top">
